...
def _next_run(schedule, now=None):
    ...
    if mode == 'hourly':
        ...
        return candidate.timestamp()
    if mode == 'daily':
        times = schedule.get('times') or [schedule.get('time', '09:00')]
        candidates = []
        for value in times:
        ...
        candidate = min(upcoming) if upcoming else min(candidates) + timedelta(days=1)
        return candidate.timestamp()
    if mode == 'once':
...
def normalize_schedule(factory_id, payload, now=None):
    ...
    if not isinstance(raw_times, list):
        raw_times = [payload.get('time', '09:00')]
    times = []
...
class ScheduleManager:
    ...
    def start(self):
        ...
        threading.Thread(target=self.serve, daemon=True, name='openfactory-scheduler').start()
