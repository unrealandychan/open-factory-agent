...
# updater removed in open source
import autostart
# licensing dummy for open-source (100% free forever)
class OpenSourceLicensing:
    FREE_WORKFLOW_LIMIT = 999999
    FREE_WORKFLOW_IDS = []
    @staticmethod
    def status(refresh=False):
        return {'ok': True, 'licensed': True, 'plan': 'open-source', 'permanent': True, 'expires_at': None}
    @staticmethod
    def free_workflow_allowed(*args, **kwargs):
        return True
    @staticmethod
    def free_workflow_ids(workflows):
        return [w.get('id') for w in (workflows or []) if isinstance(w, dict)]
    @staticmethod
    def limit_workflows(workflows, *args, **kwargs):
        return workflows
    @staticmethod
    def activate(*args, **kwargs):
        return {'ok': True, 'licensed': True, 'plan': 'open-source', 'permanent': True}
    @staticmethod
    def deactivate(*args, **kwargs):
        return {'ok': True, 'licensed': True, 'plan': 'open-source', 'permanent': True}
    @staticmethod
    def store_request(*args, **kwargs):
        return {'ok': True, 'factories': []}
licensing = OpenSourceLicensing()
from scheduler import ScheduleManager, ScheduleRunCancelled

ROOT = Path(__file__).resolve().parent
DATA_DIR = Path(os.environ.get('OPENFACTORY_DATA_DIR', ROOT)).expanduser().resolve()
DATA_DIR.mkdir(parents=True, exist_ok=True)
WORKFLOWS_FILE = DATA_DIR / 'workflows.json'
if not WORKFLOWS_FILE.exists() and (ROOT / 'workflows.default.json').exists():
    shutil.copy2(ROOT / 'workflows.default.json', WORKFLOWS_FILE)
...
STORE_PREVIEW_MODE = os.environ.get('OPENFACTORY_STORE_PREVIEW') == '1'
DEV_LICENSE_BYPASS = os.environ.get('OPENFACTORY_DEV_BYPASS_LICENSE') == '1'
PORT = int(os.environ.get('OPENFACTORY_PORT', '8765'))
HERMES_WEBHOOK_ROUTE = 'agent-task'
...
LEGACY_CONTENT_ROOT = Path.home() / 'Desktop' / 'OpenFactory'
...
if not WORKFLOWS_FILE.exists() and DATA_DIR != ROOT:
    template = ROOT / 'workflows.default.json'
    if not template.exists():
        template = ROOT / 'workflows.json'
    shutil.copy2(template, WORKFLOWS_FILE)
...
# launchd starts OpenFactory with a minimal PATH.  Keep common Homebrew
# command-line tools visible to local script cards even when the service was
# started in the background.
SCRIPT_TOOL_PATH = ':'.join(part for part in (
    '/opt/homebrew/bin', '/usr/local/bin', os.environ.get('PATH', '')
) if part)


def _prepare_script_environment(command):
    """Return an environment with tool paths and install ffmpeg on demand."""
    env = os.environ.copy()
    env['PATH'] = SCRIPT_TOOL_PATH
    if not re.search(r'(?<![A-Za-z0-9_-])ffmpeg(?:\s|$)', command):
        return env
    if shutil.which('ffmpeg', path=env['PATH']):
        return env
    brew = shutil.which('brew', path=env['PATH'])
    if not brew:
        raise RuntimeError('找不到 ffmpeg，且系統沒有 Homebrew，請先安裝 ffmpeg。')
    try:
        subprocess.run([brew, 'install', 'ffmpeg'], check=True, timeout=600,
                       stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True)
    except subprocess.CalledProcessError as exc:
        detail = (exc.stdout or '').strip()[-1000:]
        raise RuntimeError(f'自動安裝 ffmpeg 失敗：{detail}') from exc
    except subprocess.TimeoutExpired as exc:
        raise RuntimeError('自動安裝 ffmpeg 超時，請稍後重試。') from exc
    if not shutil.which('ffmpeg', path=env['PATH']):
        raise RuntimeError('ffmpeg 安裝完成但仍無法找到，請重啟 OpenFactory 後重試。')
    return env
...
def _require_workflows_write_license():
    ...
    return effective_license_status(refresh=False)


def effective_license_status(refresh=False):
    """Return development unlock status without changing real license data."""
    state = licensing.status(refresh=refresh)
    if DEV_LICENSE_BYPASS:
        return {**state, 'licensed': True, 'plan': 'development', 'development_bypass': True}
    return state
...
def _merge_limited_workflow_view(existing, submitted):
    ...
    if visible is None:
        raise ValueError('workflows.json 必須包含 workflows 陣列')
    allowed_ids = licensing.free_workflow_ids(original)
    submitted_ids = [str(item.get('id')) for item in visible if isinstance(item, dict) and item.get('id')]
    # 允許免費用戶在預設員工之外新增一個自建員工；之後仍只接受這兩個可見 ID。
    new_ids = [workflow_id for workflow_id in submitted_ids if workflow_id not in allowed_ids]
    if original and len(new_ids) == 1 and len(submitted_ids) <= licensing.FREE_WORKFLOW_LIMIT:
        # 若磁碟中還有授權版工廠，第一個非預設 ID 可能只是隱藏工廠；
        # 新增員工時以提交的 ID 取代該候選，隱藏內容仍會保留在磁碟。
        allowed_ids = [workflow_id for workflow_id in licensing.FREE_WORKFLOW_IDS if workflow_id in submitted_ids]
        allowed_ids.append(new_ids[0])
    if original and set(submitted_ids) != set(allowed_ids):
        raise PermissionError('免費版顯示模式只能修改兩個工作流（預設員工加一個自建員工）；請先匯入完整 JSON 再調整其他工作流')
    merged = dict(existing)
    merged.update({key: value for key, value in submitted.items() if key != 'workflows'})
    if not original:
        merged['workflows'] = visible
    else:
        visible_by_id = {str(item.get('id')): item for item in visible if isinstance(item, dict)}
        original_by_id = {str(item.get('id')): item for item in original if isinstance(item, dict)}
        merged['workflows'] = [visible_by_id.get(workflow_id, original_by_id.get(workflow_id, {'id': workflow_id})) for workflow_id in allowed_ids]
        merged['workflows'].extend(item for item in original if str(item.get('id')) not in allowed_ids)
    return merged
...
def migrate_legacy_content_root(copy_existing=True):
    """Move the old protected Desktop default into OpenFactory-managed storage."""
...
def check_output_path_access(value):
    ...
    probe = target / f'.openfactory-access-{uuid.uuid4().hex}.tmp'
    try:
        target.mkdir(parents=True, exist_ok=True)
        probe.write_text('OpenFactory path access test', encoding='utf-8')
        if probe.read_text(encoding='utf-8') != 'OpenFactory path access test':
...
def choose_input_file(default_location=''):
    if sys.platform != 'darwin':
        raise ValueError('目前僅支援 macOS 原生文件選擇器，請手動輸入路徑')
    script = 'POSIX path of (choose file with prompt "選擇流程輸入文件")'
    result = subprocess.run(['/usr/bin/osascript', '-e', script], capture_output=True, text=True, timeout=120, check=False)
    if result.returncode:
        if '-128' in result.stderr:
            return None
        raise RuntimeError(result.stderr.strip() or '無法開啟文件選擇器')
...
def build_webhook_headers(settings, body):
    headers = {'Content-Type': 'application/json', 'User-Agent': 'OpenFactory/1.0'}
...
def run_hermes_prompt(prompt):
    ...
    try:
        ...
        raise RuntimeError('Hermes Webhook 尚未設定，請先建立 agent-task route') from exc
    body = json.dumps({'prompt': prompt, 'event_type': 'agent-task'}, ensure_ascii=False).encode('utf-8')
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            'X-GitHub-Event': 'agent-task',
            'X-Request-ID': f'agent-{time.time_ns()}',
        },
        method='POST',
    )
    try:
...
def _run_hermes_task(task_id, prompt, session_id=''):
    ...
    try:
        command = [HERMES_BIN, 'chat', '--quiet', '--cli', '--pass-session-id', '--source', 'open-factory-agent']
        if session_id:
...
def _script_command_for_step(factory, step, values):
    ...
    tmp_dir = Path(tempfile.gettempdir()) / 'openfactory-scripts'
...
def _run_local_script_task(task_id, command, cwd, tmp_script=None):
    ...
    try:
        script_env = _prepare_script_environment(command)
        # Scripts should write directly into the step's declared output directory;
        # otherwise scripts that use os.environ.get('OUT', './output') create a
        # misleading nested output/ directory.
        script_env['OUT'] = str(cwd)
...
def workflow_topology_order(steps):
    """Return steps in dependency order, preserving JSON order for ties/cycles."""
    items = steps if isinstance(steps, list) else []
    by_id = {str(step.get('id')): step for step in items}
    positions = {str(step.get('id')): index for index, step in enumerate(items)}
    dependencies = {step_id: [] for step_id in by_id}
    for index, step in enumerate(items):
        step_id = str(step.get('id'))
        raw = step.get('dependsOn')
        raw_dependencies = raw if isinstance(raw, list) else ([] if raw is None else [raw])
        for dependency in raw_dependencies:
            key = str(dependency)
            resolved = key if key in by_id else None
            if resolved is None and key.isdigit():
                legacy_index = int(key) - 1
                if 0 <= legacy_index < len(items):
                    resolved = str(items[legacy_index].get('id'))
            if resolved and resolved != step_id and resolved not in dependencies[step_id]:
                dependencies[step_id].append(resolved)
    children = {step_id: [] for step_id in by_id}
    degree = {step_id: len(values) for step_id, values in dependencies.items()}
    for step_id, values in dependencies.items():
        for dependency in values:
            children[dependency].append(step_id)
    queue = [str(step.get('id')) for step in items if degree[str(step.get('id'))] == 0]
    ordered_ids = []
    while queue:
        step_id = queue.pop(0)
        ordered_ids.append(step_id)
        for child in sorted(children[step_id], key=positions.get):
            degree[child] -= 1
            if degree[child] == 0:
                queue.append(child)
    ordered_ids.extend(step_id for step_id in by_id if step_id not in ordered_ids)
...
def run_scheduled_factory(factory_id, schedule):
    ...
    if not effective_license_status(refresh=False)['licensed'] and not licensing.free_workflow_allowed(factory_id, factories):
        raise RuntimeError('免費版最多可使用兩個工作流（預設員工加一個自建員工）')
    ...
    ordered_steps = workflow_topology_order(steps)
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def _lan_authorized(self):
        ...
        token = next((item.split('=', 1)[1].strip() for item in cookies if item.strip().startswith('openfactory_lan=')), '')
    ...
    def _settings_payload(self, key):
        ...
        locked_factories = []
        if key == 'workflows':
            license_state = effective_license_status(refresh=False)
            limited = not license_state.get('licensed')
            if limited and isinstance(parsed.get('workflows'), list):
                allowed_ids = set(licensing.free_workflow_ids(parsed.get('workflows')))
                # 鎖定的員工仍帶完整內容，讓前端能顯示流程圖（唯讀＋蒙版），
                # 而不是整片空白。實際可否操作仍由前端鎖定與 API 限制決定。
                locked_factories = [
                    {
                        **item,
                        'id': item.get('id'),
                        ...
                        'employee_icon': item.get('employee_icon') or 'assistant_a',
                    }
                    for item in parsed['workflows']
                    if isinstance(item, dict) and str(item.get('id')) not in allowed_ids
                ]
        ...
        return {'ok': True, 'key': key, 'path': str(target), 'content': content, 'limited': limited, 'locked_factories': locked_factories}
    ...
    def do_GET(self):
        ...
        if path == '/api/license':
            ...
            self._json({'ok': True, **effective_license_status(refresh=refresh)})
            return
        if path == '/api/license/patreon/callback':
            result = parse_qs(parsed.query).get('patreon_result', [''])[0]
            try:
                if not result:
                    raise ValueError('缺少 Patreon 驗證結果')
                licensing.save_external_envelope(json.loads(result))
                message = 'Patreon 授權成功，正在返回流程工廠…'
                raw = ('<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8">'
                       '<meta name="viewport" content="width=device-width,initial-scale=1">'
                       '<meta http-equiv="refresh" content="2;url=/">'
                       '<title>Patreon 授權成功</title><style>'
                       'body{margin:0;min-height:100vh;display:grid;place-items:center;'
                       'background:#f5f6f8;color:#16181d;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}'
                       '.card{width:min(460px,calc(100vw - 40px));padding:28px;border:1px solid #e7e9ee;'
                       'border-radius:16px;background:#fff;text-align:center;box-shadow:0 18px 55px rgba(17,24,39,.12)}'
                       'a{display:inline-block;margin-top:16px;color:#2459b8}</style></head><body><main class="card">'
                       '<h1>授權成功</h1><p>' + html.escape(message) + '</p>'
                       '<a href="/">立即返回流程工廠</a></main><script>setTimeout(()=>location.href="/",2000)</script></body></html>').encode('utf-8')
                self.send_response(200)
                self.send_header('Content-Type', 'text/html; charset=utf-8')
                self.send_header('Cache-Control', 'no-store')
                self.send_header('Content-Length', str(len(raw)))
        ...
        if path == '/api/output-preview':
            try:
                ...
                if target not in declared_output_paths():
                    ...
                    return
                if target.suffix.lower() == '.html' and parse_qs(parsed.query).get('format', [''])[0] == 'html':
                    if not target.is_file(): self.send_error(404, 'HTML output not found'); return
                    source = target.read_text(encoding='utf-8')
                    token = base64.urlsafe_b64encode(str(target).encode('utf-8')).decode('ascii').rstrip('=')
                    def preview_asset(match):
                        prefix, value, suffix = match.groups()
                        if value.startswith(('http://', 'https://', 'data:', 'blob:', '#', '/')): return match.group(0)
                        return prefix + '/api/output-html-asset?html=' + token + '&path=' + quote(value, safe='') + suffix
                    source = re.sub(r'((?:src|href)=["\'])([^"\']+)(["\'])', preview_asset, source)
                    source = re.sub(r'("(?:audio|subtitles|asset)"\s*:\s*")([^"\n]+)(")', preview_asset, source)
                ...
                if target.suffix.lower() not in {'.md', '.json', '.jsonl', '.srt'}:
                    self._json({'ok': False, 'error': '目前只支援預覽 Markdown、JSON、JSONL 與 SRT 文件'}, 415)
                    return
            ...
            return
        if path == '/api/output-html-asset':
            try:
                params=parse_qs(parsed.query); token=params.get('html',[''])[0]; relative=params.get('path',[''])[0]
                html_file=Path(base64.urlsafe_b64decode(token + '=' * (-len(token)%4)).decode('utf-8')).resolve()
                if html_file not in declared_output_paths() or html_file.suffix.lower() != '.html': self.send_error(403,'Forbidden HTML preview'); return
                target=(html_file.parent/relative).resolve(); output_root=next((p for p in html_file.parents if p.name=='outputs'),html_file.parent); settings_root=Path(load_app_settings()['content_root']).expanduser().resolve(); allowed=(html_file.parent.resolve(),settings_root,output_root.resolve(),(output_root.parent/'assets').resolve())
                if not any(target==root or root in target.parents for root in allowed): self.send_error(403,'Forbidden preview asset path'); return
                if not target.is_file(): self.send_error(404,'Preview asset not found'); return
                size=target.stat().st_size; start,end=0,size-1; rh=self.headers.get('Range','')
                if rh.startswith('bytes='):
                    left,right=rh[6:].split(',',1)[0].strip().split('-',1); start=int(left) if left else max(0,size-int(right)); end=int(right) if left and right else end
                    if start<0 or start>=size or end<start: self.send_error(416,'Invalid media range'); return
                    end=min(end,size-1)
                partial=bool(rh); self.send_response(206 if partial else 200); self.send_header('Content-Type',mimetypes.guess_type(str(target))[0] or 'application/octet-stream'); self.send_header('Content-Length',str(end-start+1)); self.send_header('Accept-Ranges','bytes')
            ...
            except Exception: self.send_error(400,'Unable to read HTML preview asset')
            return
        if path == '/api/output-media':
            try:
                filepath = parse_qs(parsed.query).get('path', [''])[0]
                if not filepath:
                    self._json({'ok': False, 'error': '缺少 path 參數'}, 400)
                ...
                if target not in declared_output_paths():
                    ...
                    return
                if not target.is_file():
                    self._json({'ok': False, 'error': '文件尚未產生'}, 404)
                    return
                ...
                if range_header.startswith('bytes='):
                    try:
                        ...
                        if start < 0 or start >= size or end < start:
                            raise ValueError('範圍超出文件大小')
                        end = min(end, size - 1)
                    except (TypeError, ValueError):
                        self._json({'ok': False, 'error': '無效的媒體範圍'}, 416)
                        return
                ...
                self._json({'ok': False, 'error': f'讀取媒體文件失敗：{exc}'}, 500)
            return
        if path == '/':
    ...
    def do_POST(self):
        ...
        if parsed.path == '/api/input-file/upload':
            try:
                if not self._is_local_client():
                    raise PermissionError('拖放文件只能在執行 OpenFactory 的本機使用')
                length = int(self.headers.get('Content-Length', '0'))
                if length <= 0 or length > 48 * 1024 * 1024:
                    raise ValueError('拖入文件大小必須介於 1 byte 到 32 MB')
                payload = json.loads(self.rfile.read(length).decode('utf-8'))
                filename = Path(str(payload.get('filename') or 'dropped-file')).name or 'dropped-file'
                raw = base64.b64decode(str(payload.get('content_base64') or ''), validate=True)
                if not raw:
                    raise ValueError('拖入的文件沒有內容')
                if len(raw) > 32 * 1024 * 1024:
                    raise ValueError('拖入文件大小必須介於 1 byte 到 32 MB')
                target_dir = DATA_DIR / 'drag-drop-inputs'
                target_dir.mkdir(parents=True, exist_ok=True)
                target = target_dir / f'{uuid.uuid4().hex[:12]}-{filename}'
                target.write_bytes(raw)
                self._json({'ok': True, 'path': str(target), 'filename': filename})
            except PermissionError as exc:
                self._json({'ok': False, 'error': str(exc)}, 403)
            except (ValueError, json.JSONDecodeError, base64.binascii.Error) as exc:
                self._json({'ok': False, 'error': str(exc)}, 400)
            except Exception as exc:
                self._json({'ok': False, 'error': f'暫存拖入文件失敗：{exc}'}, 500)
            return
        ...
        if parsed.path == '/api/lan/login':
            ...
            try:
                ...
                self.send_header('Content-Type', 'application/json; charset=utf-8')
                self.send_header('Cache-Control', 'no-store')
                self.send_header('Set-Cookie', f'openfactory_lan={lan_session_token()}; Path=/; HttpOnly; SameSite=Strict')
                self.send_header('Content-Length', str(len(raw)))
                self.end_headers()
                self.wfile.write(raw)
            except PermissionError as exc:
        ...
        if parsed.path == '/api/uninstall':
            try:
                ...
                uninstall_cmd = (
                    '#!/bin/sh\n'
                    'sleep 1\n'
                    'U="$HOME/.openfactory/current/scripts/uninstall.sh"\n'
                    'if [ -x "$U" ]; then "$U"; else "$HOME/.local/bin/openfactory" uninstall; fi\n'
                    'rm -f "$HOME/.openfactory/uninstall-last.log"\n'
                )
                marker = Path.home() / '.openfactory' / 'uninstall-pending.sh'
        ...
        if parsed.path == '/api/output-path/select':
            try:
                if not self._is_local_client():
                    raise PermissionError('請在執行 OpenFactory 的本機選擇資料夾')
                length = int(self.headers.get('Content-Length', '0'))
                default_path = ''
                if length > 0:
                    if length > 8192:
        ...
        if parsed.path == '/api/input-file/select':
            try:
                if not self._is_local_client():
                    raise PermissionError('請在執行 OpenFactory 的本機選擇文件')
                path = choose_input_file()
                self._json({'ok': True, 'cancelled': path is None, 'path': path or ''})
            except PermissionError as exc:
                self._json({'ok': False, 'error': str(exc)}, 403)
            except (ValueError, RuntimeError, subprocess.TimeoutExpired) as exc:
                self._json({'ok': False, 'error': str(exc)}, 400)
        ...
        if parsed.path == '/api/agent/run':
            try:
                ...
                if not effective_license_status(refresh=False)['licensed']:
                    stored = json.loads(WORKFLOWS_FILE.read_text(encoding='utf-8'))
                    if not licensing.free_workflow_allowed(factory_id, stored.get('workflows')):
                        raise PermissionError('免費版最多可執行兩個工作流（預設員工加一個自建員工），請升級授權')
        ...
        if parsed.path == '/api/script/run':
            try:
                if not self._is_local_client():
                    raise PermissionError('本機腳本只能在執行 OpenFactory 的電腦啟動')
                length = int(self.headers.get('Content-Length', '0'))
                ...
                if not effective_license_status(refresh=False)['licensed']:
                    if not licensing.free_workflow_allowed(factory_id, factories):
                        raise PermissionError('免費版最多可執行兩個工作流（預設員工加一個自建員工），請升級授權')
        ...
        if parsed.path == '/api/schedule':
            try:
                ...
                if payload.get('enabled') is True and not effective_license_status(refresh=False)['licensed']:
                    if not licensing.free_workflow_allowed(factory_id, factories):
                        raise PermissionError('免費版最多可為兩個工作流設定自動化（預設員工加一個自建員工）')
        ...
        if parsed.path == '/api/agent/disconnect':
            ...
            return
        if parsed.path in {'/api/license/activate', '/api/license/deactivate', '/api/license/patreon/start', '/api/license/patreon/disconnect'}:
            try:
                if parsed.path.endswith('/patreon/start'):
                    host = self.headers.get('Host', '').strip()
                    callback_uri = f"http://{host}/api/license/patreon/callback" if host else f"http://127.0.0.1:{PORT}/api/license/patreon/callback"
                    self._json({'ok': True, **licensing.patreon_start(callback_uri)})
                    return
                if parsed.path.endswith('/deactivate'):
                    self._json({'ok': True, **licensing.deactivate()})
                    return
                if parsed.path.endswith('/patreon/disconnect'):
                    licensing.disconnect_patreon()
                    self._json({'ok': True, **licensing.status(refresh=False)})
                    return
                length = int(self.headers.get('Content-Length', '0'))
                if length <= 0 or length > 4096:
                    raise ValueError('請求大小必須介於 1 byte 到 4 KB')
        ...
        if parsed.path in {'/api/store/install', '/api/store/upload', '/api/store/mine', '/api/store/unpublish'}:
            try:
                ...
                if parsed.path.endswith('/mine'):
                    if STORE_PREVIEW_MODE:
                        if not effective_license_status(refresh=False).get('licensed'):
        ...
        if parsed.path in {'/api/update/check', '/api/update/install'}:
            try:
                ...
                if parsed.path == '/api/update/check':
                    self._json({'ok': True, 'update_available': False, 'current_version': VERSION, 'latest_version': VERSION, 'message': '已是最新開源版本'})
                    return
                self._json({'ok': True, 'updated': False, 'message': '開源版本請透過 git pull 或 release 安裝更新', 'version': VERSION})
                return
            except (ValueError, json.JSONDecodeError) as exc:
                self._json({'ok': False, 'error': str(exc)}, 400)
            except Exception as exc:
        ...
        try:
            ...
            if key == 'workflows' and payload.get('limited_view') is True and not effective_license_status(refresh=False).get('licensed'):
            ...
            self._json({
                ...
                'limited': key == 'workflows' and not effective_license_status(refresh=False).get('licensed'),
                'message': 'JSON 已儲存',
            })
