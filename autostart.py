...
LABEL = "com.gda.openfactory"
INSTALL_ROOT = Path(os.environ.get("OPENFACTORY_INSTALL_ROOT", Path.home() / ".openfactory")).expanduser().resolve()
PLIST_FILE = Path.home() / "Library" / "LaunchAgents" / f"{LABEL}.plist"
LOG_DIR = Path.home() / "Library" / "Logs" / "OpenFactory"
...
def _enable(port: int) -> None:
    ...
    payload = {
        ...
        "EnvironmentVariables": {
            "OPENFACTORY_INSTALL_ROOT": str(INSTALL_ROOT),
            "OPENFACTORY_DATA_DIR": str(INSTALL_ROOT / "data"),
            "OPENFACTORY_PORT": str(port),
            "OPENFACTORY_PYTHON": sys.executable,
